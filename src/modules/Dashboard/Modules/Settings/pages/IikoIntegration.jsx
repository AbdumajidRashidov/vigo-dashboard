...
import { AddIikoFilialsDrawer } from "../components/Drawers/IikoFilialsDrawer";
import { AddIikoMenuDrawer } from "../components/Drawers/IikoMenuDrawer";
import { AddIikoPOSModal } from "../components/Modals/IikoPOSModal";

const IikoIntenration = () => {
  const [tab, setTab] = useState("Filiallar");
  const [isUpdate, setIsUpdate] = useState(false);

  const modalFilials = useOverlay({
    uniqueName: "addIikoFilialsModal",
    onClose: () => setIsUpdate(false),
  });
  const modalMenu = useOverlay({
    uniqueName: "addIikoMenuModal",
    onClose: () => setIsUpdate(false),
  });
  const modalIikoPOS = useOverlay({
    uniqueName: "addIikoPOSModal",
    onClose: () => setIsUpdate(false),
  });
  ...
  return (
    <>
      <AddIikoPOSModal
        isOpen={modalIikoPOS.isOverlayOpen}
        handleModalClose={modalIikoPOS.handleOverlayClose}
        onSuccess={() => {
          modalIikoPOS.handleOverlayClose();
        }}
      />
      <AddIikoFilialsDrawer
        ...
        actions={
          tab == "Filiallar" ? (
            <Button
              design="primary"
              className="btn"
              onClick={modalIikoPOS.handleOverlayOpen}
              text=" + O'rnatish"
            />
          ) : (
            ""
...
export default IikoIntenration;
